...
title: Set up Strada in a fullstack app
...
icon: lucide:rocket
prompt: |
  Write the Quickstart from @/sdk/src/, @/website/src/docs/quickstart.mdx,
  and @/example-app/src/index.ts.
  Cover browser vs server init, STRADA_TOKEN vs public projectId, and first-query steps.
---
...
On Node, the SDK registers process handlers so buffered telemetry flushes on `beforeExit`, `SIGTERM`, `SIGINT`, and `uncaughtException`. You usually do not call `flush()` yourself. See the [SDK reference](./sdk.mdx) for the exit-path details.

### Cloudflare Workers

Workers expose bindings through `cloudflare:workers`. Initialize at module
scope, not inside request middleware:
...
Use one Strada project for related services. Give each Worker a distinct
`service` and set `environment` to `development`, `preview`, or `production`
so the same queries can compare deployments.

`env.STRADA_PROJECT_ID` is empty until the secret is uploaded, and it is always
...
telemetry off explicitly. See [Turning telemetry off](./sdk.mdx#turning-telemetry-off).
...
- `session.id`, `visitor.id`, and `user.id` injected into every span/log. Only `session.id` and `user.id` are propagated to the server
...
If a framework owns the root document and exposes no client component slot, do
not force this component into an unrelated wildcard layout. Keep server-side
instrumentation only until the framework provides a supported client hook.
...
Keep route and handler tags low-cardinality. **Email and user id are fine** on
errors, logs, and product events. That is how you find who hit an issue. Do not
attach API keys, prompts, or the raw text the user typed.
...
Once the browser knows who the user is, set the `strada_uid` cookie (JS-readable) on login, or call `identifyUser({ id })`. Unique visitors use a separate `strada_vid` cookie. Logout clears `strada_uid` and leaves `strada_vid`. The SDK **propagates `user.id` to the server via W3C Baggage**, so server spans and logs for that request also carry the user id. `session.id`, `visitor.id`, and `user.id` are injected into browser spans and logs. Only `session.id` and `user.id` are propagated to the server.

For server-first requests there is no browser header to read, so put the user into baggage yourself in auth middleware. That works with any auth library; the Better Auth plugin just does it for you. See [Server-side user identification without Better Auth](./sdk.mdx#server-side-user-identification-without-better-auth) for the middleware, the `withUser()` wrapper, Workers, and the `identifyUser()` profile snapshot.
