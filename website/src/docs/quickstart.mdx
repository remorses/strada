...
telemetry off explicitly. See [Turning telemetry off](../sdk/README.mdx#turning-telemetry-off).
...
For server-first requests there is no browser header to read, so put the user into baggage yourself in auth middleware. That works with any auth library; the Better Auth plugin just does it for you. See [Server-side user identification without Better Auth](../sdk/README.mdx#server-side-user-identification-without-better-auth) for the middleware, the `withUser()` wrapper, Workers, and the `identifyUser()` profile snapshot.
