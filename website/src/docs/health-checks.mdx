...
title: Monitor URLs with health checks
...
prompt: |
  Write the health checks guide from @/cli/src/checks.ts,
  @/website/src/health-check-dispatch.ts, and @/website/src/health-check-workflow.ts.
---
...
| `--project` | first project | Project that stores the check results |
...
```bash
# List all health checks with state, last run, status code, latency, and 24h uptime
strada checks list

# Recent runs of one check, plus the response body of the last failure
strada checks view 01KPVGTT9CJW4ZNEF414VHGRFD
...
`checks list` shows one **state** per check:

| State | Meaning |
|-------|---------|
| `pending` | No run yet. The check runs on the next matching cron tick |
| `up` | Last run passed |
| `down` | Last run failed, not enough consecutive failures to alert yet |
| `alerting` | Consecutive failure threshold reached, alert sent |
| `disabled` / `auto-disabled` | Not running. Re-enable with `strada checks enable <id>` |
...
strada alerts create --name "On-call email" --channel email --to oncall@example.com
strada alerts create --name "Slack" --channel webhook --to https://hooks.slack.com/services/...
```
...
See [Alerts and Destinations](./alerts.mdx) for the full guide on setting up email, webhook, and Slack destinations.
