...
title: Browser analytics with OpenTelemetry
sidebarTitle: Browser Analytics
...
prompt: |
  Write the browser analytics guide from @/sdk/src/browser.ts,
  @/tinybird/datasources/, and @/cli/src/analytics.ts.
---
...
- **Visitor** = one `visitor.id`, cookie `strada_vid`. Survives tabs, login, and logout. This is unique visitors.
...
- **First visit** = `visitor.first_visit=true` on the first pageview of a new `visitor.id`
...
Strada reads `navigator.userAgentData`, `navigator.language`, and `navigator.userAgent` during `initStrada()` and sets these **resource attributes**:
...
| `visitor.id`                    | cookie `strada_vid`                    | `"f47ac10b-58cc-..."`           |
| `visitor.first_visit`           | true on the first pageview of a new visitor | `"true"`                   |
...
| `user.id`                       | cookie `strada_uid`                    | `"user_123"`                    |
...
- Per-tab, per-origin
- `session.id` lives in `sessionStorage`. No cookie
- Unique visitors use `visitor.id`: first-party cookie `strada_vid`, 1 year. Login writes `strada_uid` for the account. Logout clears `strada_uid` and leaves `strada_vid`. Data stays in the project's own Tinybird/ClickHouse. No third-party tracker
...
These come from Strada's inline browser detection in `initStrada()`.
...
- `session.id`, `visitor.id`, `url.path`, `user.id` are injected automatically by the SDK's log processor
...
This span is created automatically by `@opentelemetry/instrumentation-fetch` when that optional package is installed. The `session.id` and `visitor.id` are injected by the custom span processor.

---

## Auto-instrumentation

Pageviews are automatic. Optional `@opentelemetry/auto-instrumentations-web` can add fetch, XHR, document-load, and click spans when you install that package:
...
- Does **not** start a new pageview when the tab becomes visible again
...
`strada.track()` emits a log record via the OTel Logs API. The log record lands in `otel_logs` with `event.name` set to the event name. Context (`session.id`, `visitor.id`, `url.full`, `user.id`) is injected automatically. If a pageview span is currently open, `TraceId` and `SpanId` are attached too. Browser auto-instrumented spans also inherit that pageview as parent unless some other span is already active. Developers only pass event-specific properties.
...
2. Generates or restores `visitor.id` from cookie `strada_vid`
3. Initializes `WebTracerProvider` with `BatchSpanProcessor`
4. Attaches a span processor that injects `session.id`, `visitor.id`, and `user.id` onto every span
...
|---|---|---|
| `otel_analytics_pages` | Top pages, browsers, bots, devices, countries, languages, referrers, pageview histogram | `ProjectId, ServiceName, Domain, Date, Device, Browser, Country, Language, Pathname, Referrer` |
| `otel_analytics_sessions` | Bounce rate, avg session duration | `ProjectId, ServiceName, Domain, Date, SessionId` |

### `otel_analytics_pages`

Pre-aggregates pageview counts and unique visitors by page, device, browser, country, language, referrer, and **domain** (the host the user is visiting).
...
Visits          AggregateFunction(uniq, String)    -- uniqState(visitor.id), session.id fallback
FirstVisits     AggregateFunction(uniq, String)    -- uniqStateIf(visitor.id, first_visit)
...
- [x] Add `otel_analytics_pages` and `otel_analytics_sessions` to the JWT `scopes` array with `ProjectId` filter (same pattern as other tables)
