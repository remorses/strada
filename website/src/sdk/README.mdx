...
Set the same cookie from your backend after login if you already have auth middleware. This keeps the user ID available before browser code runs and makes the value stable across page loads. Any auth library works, the SDK only reads the cookie:

```ts
app.use(async (req, res, next) => {
  const user = await getUserFromRequest(req) // your own session lookup
  if (user) {
...
app.use(async (req, res, next) => {
  ...
  // Optional: lets the browser SDK tag frontend telemetry with the same id
  res.setHeader("Set-Cookie", `strada_uid=${encodeURIComponent(user.id)}; Path=/; SameSite=Lax; Secure; Max-Age=31536000`)
