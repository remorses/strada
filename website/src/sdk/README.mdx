...
When login/logout happens after initialization, call **`identifyUser()`** in the browser. Browser calls persist `user.id` in cookie `strada_uid` and update in-memory context. Cookie `strada_vid` is the visitor and is left alone. They do not store email/name/profile data.
...
In the browser, the log record is automatically correlated to the active pageview span via `TraceId`/`SpanId`, and context attributes (`session.id`, `url.path`, `user.id`) are injected for you. On the server there is no pageview span, so pass the identifiers you want to filter by as properties (`user_id`, `org_id`, ...). Use the same key names on both sides so one query covers them.
